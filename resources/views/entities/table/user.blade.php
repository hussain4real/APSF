<div class="max-w-2xl sm:max-w-sm md:max-w-sm lg:max-w-sm xl:max-w-xl bg-white text-gray-900">
    ...
    <div class="flex-shrink-0 m-2 relative overflow-hidden bg-primary-500 rounded-lg max-w-xs shadow-lg group">
        ...
        <div class="relative pt-10 px-10 flex items-center justify-center group-hover:scale-110 transition-transform">
            ...
            @unless ($getRecord()->media && $getRecord()->media->count() > 0)
            ...
            @else
            <img class="relative max-w-48 max-h-48 w-48 h-48 rounded-lg bg-transparent" src="{{$getRecord()->profile_photo_url}}" alt="">

            @endunless

        </div>
        <div class="relative text-white px-2 pb-2 mt-6">
            <span class="block opacity-75 mb-1">verified:
                <span class="text-teal-900 opacity-100 font-bold">
                    {{-- {{$getRecord()->user->email_verified_at ? 'Yes' : 'No'}}--}}
                    {{$getRecord()?->email_verified_at?->diffForHumans() ?? 'No'}}
                </span>
            </span>
            <span class="block font-semibold text-xl text-wrap">{{$getRecord()->name}}</span>
            <div class="flex justify-between items-center mt-2">
                <span class="block text-sm font-semibold bg-white rounded-full text-teal-500 px-3 py-2 leading-none">
                    {{$getRecord()->profile_type}}
                </span>
