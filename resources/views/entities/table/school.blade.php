<div class="max-w-2xl sm:max-w-sm md:max-w-sm lg:max-w-sm xl:max-w-xl bg-white text-gray-900">
    ...
    <!-- profile card start -->
    <div class="flex-shrink-0 relative overflow-hidden bg-primary-500 rounded-lg max-w-xs shadow-lg group">
        ...
        <div class="relative text-white px-2 pb-2 mt-6">
            <span class="block opacity-75 mb-1">{{$getRecord()->type}}</span>
            <span class="block font-semibold text-xl text-wrap">{{$getRecord()->school_name}}</span>
            <div class="flex justify-between items-center mt-2">
                <span class="block text-xs font-semibold">{{$getRecord()->city}}<span class="text-sm">, {{$getRecord()->state}}</span></span>
